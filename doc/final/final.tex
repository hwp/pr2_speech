...
To sum it up,  \textit {speech recognition} application is a simple speech
...
\begin{center}
  \begin{tikzpicture}[node distance=1cm, auto, ->, thick,
    filter/.style={rectangle, draw=black, very thick, text centered, minimum size=25pt},
    operator/.style={circle, draw=black, very thick, text centered, minimum size=12pt, inner sep=0},
    ]
    \node (n1) {$y(t)$};
    \node (n2)[filter, right=of n1] {STFT};
    \node (n3)[filter, right=of n2] {$|Y(n,k)|^b$};
    \node (n4)[operator, right=of n3] {$-$};
    \node (n5)[operator, right=of n4] {$+$};
    \node (n6)[filter, right=of n5] {Inverse STFT};
    \node (n7)[right=of n6] {$\hat{x}(t)$};
    \node (n8)[filter, above=of n3] {Phase$(Y(n,k))$};
    \node (n9)[operator, regular polygon,regular polygon sides=3, below=of n4] {$\alpha$};
    \node (n0)[filter, below=of n9] {$\hat{N}(k))$};
